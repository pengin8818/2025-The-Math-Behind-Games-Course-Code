...
while run:
    ...
    if goal.collide((lander.position[0], lander.position[1])):
        if lander.get_speed() < max_speed:
            goal.move_goal((rand.uniform(10,600), rand.uniform(320,600)))
            points += 1
        else:
            run = False
    ...
    screen.blit(goal_surface, (goal.position[0] - goal.size[0]/2, goal.position[1] - goal.size[1]/2))
